...
public class Personas extends Abm<Persona, Abm.Model> {
    ...
    private void configureGrid(Grid<Persona> grid) {
        grid.addColumn(Persona::getNombre).setHeader("Nombre").setWidth("50%").setKey("nombre").setSortable(true);
        grid.addColumn(Persona::getDocumento).setHeader("Documento").setKey("documento").setWidth("30%").setSortable(true);
